...
5. **Export / reset:** triple-tap the top-right corner of any screen (or the
   ...
   (default `2468` — change `PASSCODE` in `js/screens/researcher.js` before
   running sessions), then **Export CSV** (opens the iOS share sheet →
   AirDrop / Files / Mail; a raw-JSON backup and a copy-to-clipboard fallback
   are also there) and **Reset for next participant**.
