...
import { TESTING_TOOLS } from './config.js';
import { clear, el, onTripleTap } from './dom.js';
...
// Testing only — see js/config.js. Wipes the session and returns to the title
// screen in one tap. Must be switched off before running participants.
if (TESTING_TOOLS) {
  document.body.append(el('button', {
    id: 'restart-button',
    text: '↺ Restart',
    title: 'Testing build — wipes this session',
    on: { click: () => app.resetForNextParticipant() },
  }));
}
